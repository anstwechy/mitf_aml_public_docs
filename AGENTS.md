...
- Navigation: sidebar-first (no `navigation.tabs`); **do not** use `toc.integrate` if the left nav feels crowded—keep the page table of contents in the **right** column on wide screens. Theme matches **MITF Wallet** public docs: **teal** / **green** + **deep orange** accent, **Inter** body / **Fira Code** mono, Masarat light/dark logos via `overrides/partials/logo.html`, custom `docs/stylesheets/extra.css` (default Material sidebar; no `navigation.expand` / `navigation.sections` / `navigation.indexes`).
